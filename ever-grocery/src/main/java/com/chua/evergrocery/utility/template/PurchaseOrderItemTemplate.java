 ...
 * @since   Apr 10, 2019
 */
public class PurchaseOrderItemTemplate extends AbstractTemplate {
	...
	public PurchaseOrderItemTemplate(PurchaseOrderDetail purchaseOrderItem) {
		this.purchaseOrderItem = purchaseOrderItem;
		this.content = purchaseOrderItem.getProductDetail().getContent();
	...
	public String merge(VelocityEngine velocityEngine, DocType docType) {
		...
		model.put("t", this);
		return VelocityEngineUtils.mergeTemplateIntoString(velocityEngine, docType.getFolderName() + "/purchaseOrderItem.vm", "UTF-8", model);
	}
	
	public String getFormattedQuantity() {
		NumberFormat nf = new DecimalFormat("##.#");
		return String.format("%-4s", nf.format(purchaseOrderItem.getQuantity()));
	}
	
	public String getFormattedName() {
		String formattedName = purchaseOrderItem.getUnitType().getShorthand() + " ";
		
		NumberFormat nf = new DecimalFormat("##.00");
		
		formattedName += purchaseOrderItem.getProductName();
		if((purchaseOrderItem.getQuantity() % 1.0f == 0.5f
				|| purchaseOrderItem.getUnitType().equals(UnitType.CASE)
				|| purchaseOrderItem.getUnitType().equals(UnitType.BUNDLE)
				|| purchaseOrderItem.getUnitType().equals(UnitType.SACK))
				&& !content.equals(0)) {
			formattedName += "x" + content;
		}
		
		if(purchaseOrderItem.getQuantity() > 1) {
			formattedName += " @" + nf.format(purchaseOrderItem.getNetPrice());
		}
	...
	public String getFormattedTotalPrice() {
		return String.format("%10s", purchaseOrderItem.getFormattedTotalPrice());
	}
