...
public class PurchaseOrderHandlerImpl implements PurchaseOrderHandler {
	...
	public ResultBean generatePurchaseOrder(Long companyId, Float daysToBook) {
		...
		if(daysToBook >= 3) {
			...
			if(Days.daysBetween(new DateTime(company.getLastPurchaseOrderDate()), new DateTime()).getDays() > 3) {
				LOG.info("#### Generating purchase order for " + company.getName() + " - " + DateFormatter.longFormat(generateStartTime));
				...
				if(lastPODate.getTimeInMillis() == DateUtil.getDefaultDateInMillis()) {
					lastPODate.setTime(purchaseOrderService.findLatestDeliveryByCompany(companyId).getDeliveredOn());
				}
				...
				final List<InventoryBean> inventories = inventoryHandler.getProductInventoryByCompany(companyId, generateStartTime);
				...
				for(InventoryBean inventory : inventories) {
					...
					final Float proportionedNetSalesAmount = (inventory.getTotalBaseSales() < actualBudget) ? inventory.getTotalBaseSales() / salesPeriod * company.getDaysBooked() : inventory.getTotalBaseSales();
				...
				final String fileName = StringHelper.convertToFileSafeFormat(company.getName()) + "_purchase.order_" + DateFormatter.fileSafeShortFormat(new Date()) + ".pdf";
	...
	public ResultBean generateOfftake(Long companyId, Float offtakeDays, Boolean download, Boolean print) {
		...
		if(company != null) {
			...
			for(Product product : products) {
				final Double averageOfftake = (transactionSummaryHandler.getProductDailySalesSummaryList(product.getId(), daysToAverage)
												.stream().mapToDouble(pdss -> pdss.getBaseTotal())
												.sum()) / ((float) daysToAverage);
	...
	public ObjectList<PurchaseOrderDetail> getPurchaseOrderDetailList(Integer pageNumber, Long purchaseOrderId) {
		return purchaseOrderDetailService.findAllWithPagingOrderByLastUpdate(pageNumber, UserContextHolder.getItemsPerPage(), purchaseOrderId);
	}
